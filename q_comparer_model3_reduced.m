...
legend('q-rad', 'q-evap-top', 'q-glass2air');
...
function dTMdt = derivate(p, TM_l, comp,t)
    ...
    dTMdt(1) = -1/(cp_water(T_l)*rho_water(T_l)*p.volume_l)*(q_rad_side(T_l,p) + q_rad_top(T_l,p) + q_evap_top(T_l, p,0.5) + q_glass2air(T_l,p,0.5) );
...
function q_values_calc(T_out_cup, T_top, p)
    ...
    time_values = [time_values, t_current];
    q_values = [q_rad_side(T_out_cup,p) + q_rad_top(T_top,p), q_evap_top(T_top,p,0.5), q_glass2air(T_out_cup,p,0.5)];

    fileID = fopen('q_values.csv','a');
    if ~exist('q_values.csv', 'file')
        fprintf(fileID, '%s,%s,%s,%s\n', 'Time', 'q_rad', 'q_evap_top', 'q_glass2air');
    end
    ...
    if ~exist('relq_values.csv', 'file')
        fprintf(fileID, '%s,%s,%s,%s\n', 'Time', 'relq_rad', 'relq_evap_top', 'relq_glass2air');
    end
